...
function get_client_ip_address (){
  ...
  foreach (array ('HTTP_CF_CONNECTING_IP',
                  'HTTP_CLIENT_IP',
...
function check_ip_address_list ($ip_addresses, $white_list) {
  ...
  if (trim ($ip_addresses) == AD_EMPTY_DATA) return !$return;
...
function check_country_list ($countries, $white_list) {
  ...
  if (trim ($countries) == AD_EMPTY_DATA) return !$return;
  $countries = explode (",", $countries);

  $country_from_ip_address = ip_to_country (get_client_ip_address ());
  ...
  foreach ($countries as $country) {
    ...
    } elseif ($country == $country_from_ip_address) return $return;
  }
  return !$return;
}
